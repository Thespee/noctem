...
# AI-related methods (v0.6.0)

def get_unscored_tasks(limit: int = 10) -> list[Task]:
    """Get tasks that haven't been AI-scored yet."""
    with get_db() as conn:
        rows = conn.execute(
            """
            SELECT * FROM tasks 
...
def update_ai_score(task_id: int, score: float) -> Optional[Task]:
    """Update the AI help score for a task."""
    with get_db() as conn:
        conn.execute(
            """
            UPDATE tasks 
    ...
    log_action("task_ai_scored", "task", task_id, {"score": score})
    return get_task(task_id)


def get_ai_ready_tasks(min_score: float = 0.5) -> list[Task]:
    """Get tasks with high AI help scores that are ready for processing."""
    with get_db() as conn:
        rows = conn.execute(
            """
            SELECT * FROM tasks 
...
def get_tasks_with_intentions() -> list[dict]:
    """Get tasks that have implementation intentions."""
    with get_db() as conn:
        rows = conn.execute(
            """
            SELECT t.*, i.id as intention_id, i.when_trigger, i.where_location, 
                   i.how_approach, i.first_action, i.status as intention_status
            FROM tasks t
            JOIN implementation_intentions i ON t.id = i.task_id
...
def get_current_next_step(task_id: int) -> Optional[dict]:
    """Get the current next step for a task."""
    with get_db() as conn:
        row = conn.execute(
            """
...
def complete_next_step(step_id: int) -> bool:
    """Mark a next step as completed and activate the next one."""
    with get_db() as conn:
        # Get the step
        row = conn.execute(
            "SELECT * FROM next_steps WHERE id = ?",
            (step_id,)
        ).fetchone()
        ...
        # Mark as completed
        conn.execute(
            "UPDATE next_steps SET status = 'completed', completed_at = CURRENT_TIMESTAMP WHERE id = ?",
            (step_id,)
        )
        ...
        conn.execute(
            ...
            UPDATE next_steps 
            SET status = 'current' 
            WHERE task_id = ? AND step_order = ? AND status = 'pending'
            """,
            (row['task_id'], row['step_order'] + 1)
        )
