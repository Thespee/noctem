#!/usr/bin/env python3
"""
Noctem v0.6.0 - Executive Assistant System with AI
Main entry point.
"""
import argparse
import asyncio
...
def run_ai_loop():
    """Run the AI loop standalone."""
    from .ai.loop import AILoop
    from .config import Config
    
    loop = AILoop(poll_interval=Config.get('ai_poll_interval', 30))
...
def main():
    ...
    parser.add_argument(
        "mode",
        choices=["bot", "web", "cli", "all", "ai", "init"],
        default="cli",
        nargs="?",
        help="Run mode: bot (Telegram), web (dashboard), cli (terminal), ai (AI loop), all, or init (setup DB)",
    )
    parser.add_argument(
        "--port",
        type=int,
    ...
    if args.mode == "cli":
        ...
        # Run AI loop in thread if enabled
        if Config.get('ai_enabled', True):
            from .ai.loop import AILoop
            ai_loop = AILoop(poll_interval=Config.get('ai_poll_interval', 30))
            ai_loop.start(daemon=True)
            logger.info("AI loop started in background")
