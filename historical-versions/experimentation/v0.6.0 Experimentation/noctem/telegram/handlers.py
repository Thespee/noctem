...
async def cmd_nextaction(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /nextaction command - show next action for a task."""
    if not context.args:
        # Show next action for highest priority task with an intention
        tasks_with_intentions = task_service.get_tasks_with_intentions()
        if not tasks_with_intentions:
            await update.message.reply_text("No tasks have AI breakdowns yet. Add some tasks and wait for processing.")
            return
        
        item = tasks_with_intentions[0]
        msg = f"""🎯 **Next Action**

**Task:** {item['task'].name}
**First step:** {item['first_action']}

**When:** {item['when_trigger']}
**Where:** {item['where_location']}

_Type `done {item['task'].name}` when complete_"""
        await update.message.reply_text(msg, parse_mode="Markdown")
        return
    
    # Get next action for specific task
    ...
    next_step = task_service.get_current_next_step(task.id)
    if not next_step:
        await update.message.reply_text(f"No next action defined for: {task.name}")
        return
    
    await update.message.reply_text(f"🎯 **Next Action for {task.name}:**\n\n{next_step['step_text']}", parse_mode="Markdown")


async def cmd_aistatus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /aistatus command - show AI system status."""
    try:
        from ..ai.loop import AILoop
        from ..ai.degradation import GracefulDegradation
        
        degradation = GracefulDegradation()
        health = degradation.check_health()
        
        # Get pending work count
        pending = degradation.get_pending_work(limit=100)
...
{status_emoji.get(health.level, '⚪')} **Level:** {health.level.upper()}
{health.message}

**Ollama:** {'✓ Running' if health.ollama_available else '✗ Not available'}
**Fast model:** {'✓ Loaded' if health.fast_model_loaded else '✗ Not loaded'}
**Slow model:** {'✓ Loaded' if health.slow_model_loaded else '✗ Not loaded'}

**Pending work:** {len(pending)} items"""
        
        await update.message.reply_text(msg, parse_mode="Markdown")
    except Exception as e:
        await update.message.reply_text(f"Error checking AI status: {str(e)}")


async def cmd_aisettings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /aisettings command - configure AI settings."""
    from ..config import Config
...
**Confidence threshold:** {Config.get('ai_confidence_threshold', 0.5)}
**Poll interval:** {Config.get('ai_poll_interval', 30)}s
**Fast model:** {Config.get('fast_model')}
**Slow model:** {Config.get('slow_model')}
**Ollama host:** {Config.get('ollama_host')}

_Use `/aisettings <key> <value>` to change_"""
        await update.message.reply_text(msg, parse_mode="Markdown")
        return
    ...
    if len(context.args) >= 2:
        ...
        # Validate key
        allowed_keys = ['ai_enabled', 'ai_confidence_threshold', 'ai_poll_interval']
        ...
        if key == 'ai_enabled':
            ...
            value = float(value)
        elif key in ('ai_poll_interval',):
            value = int(value)
        
        Config.set(key, value)
        await update.message.reply_text(f"✓ Set {key} = {value}")
    else:
        await update.message.reply_text("Usage: /aisettings <key> <value>")
