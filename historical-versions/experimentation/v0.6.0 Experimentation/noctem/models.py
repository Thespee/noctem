...
class Task:
    ...
    # AI fields (v0.6.0)
    ai_help_score: Optional[float] = None  # 0-1, NULL = not scored
    ...
    def from_row(cls, row) -> "Task":
        ...
        try:
            ai_help_score = row["ai_help_score"]
            ai_processed_at = row["ai_processed_at"]
        except (KeyError, IndexError):
            pass  # Old schema without AI fields
...
class ImplementationIntention:
    """A breakdown of how to accomplish a task."""
    id: Optional[int] = None
    task_id: Optional[int] = None
    version: int = 1
    when_trigger: Optional[str] = None  # When will you start?
    where_location: Optional[str] = None  # Where will you do it?
    how_approach: Optional[str] = None  # How will you approach it?
    first_action: Optional[str] = None  # What's the very first step?
    generated_by: str = "llm"  # 'llm' or 'user_edited'
    ...
    @classmethod
    def from_row(cls, row) -> "ImplementationIntention":
        if row is None:
            return None
        return cls(
            id=row["id"],
            task_id=row["task_id"],
            version=row["version"],
            when_trigger=row["when_trigger"],
            where_location=row["where_location"],
            how_approach=row["how_approach"],
...
@dataclass
class NextStep:
    """A single next action extracted from an implementation intention."""
    id: Optional[int] = None
    task_id: Optional[int] = None
...
class ClarificationRequest:
    """A question the AI needs answered about a task."""
    id: Optional[int] = None
    task_id: Optional[int] = None
    question: str = ""
    ...
    @classmethod
    def from_row(cls, row) -> "ClarificationRequest":
        if row is None:
            return None
        options = []
        if row["options"]:
    ...
    def options_json(self) -> str:
        """Return options as JSON string for DB storage."""
        return json.dumps(self.options) if self.options else None


@dataclass
class PendingSlowWork:
    """A queued AI task for processing when resources are available."""
    id: Optional[int] = None
    task_type: str = ""  # e.g., 'implementation_intention', 'clarification'
    task_id: Optional[int] = None
    ...
    @classmethod
    def from_row(cls, row) -> "PendingSlowWork":
        if row is None:
            return None
        task_data = {}
        if row["task_data"]:
    ...
    def task_data_json(self) -> str:
        """Return task_data as JSON string for DB storage."""
        return json.dumps(self.task_data) if self.task_data else None
